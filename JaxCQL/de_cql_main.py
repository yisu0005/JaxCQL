...
from .model import TanhGaussianPolicy, FullyConnectedQFunction, SamplerPolicy, Discriminator, ActionOnlyRepresentationPolicy, ActionRepresentationPolicy
...
FLAGS_DEF = define_flags_with_default(
    ...
    train_bc=True,
    train_cql=True,
    encoder_no_tanh=False,
    action_scale=1.0,
    discriminator_arch='256-256',
...
def main(argv):
    ...
    if FLAGS.train_cql:
        ...
        qf = FullyConnectedQFunction(observation_dim, latent_action_dim, FLAGS.qf_arch, FLAGS.orthogonal_init)

        if FLAGS.cql.target_entropy >= 0.0:
            FLAGS.cql.target_entropy = -(np.prod(eval_sampler.env.action_space.shape) * FLAGS.policy_entropy_scale).item()
        

        decql = DECQL(FLAGS.cql, policy, qf, encoder, discriminator, bc_agent)
        ...
        for epoch in range(FLAGS.n_epochs):
            ...
            with Timer() as train_timer:
                for batch_idx in range(FLAGS.n_train_step_per_epoch):
                    batch = batch_to_jax(subsample_batch(dataset, FLAGS.batch_size))
                    metrics.update(prefix_metrics(decql.train(batch, bc=epoch < FLAGS.bc_epochs), 'decql'))
