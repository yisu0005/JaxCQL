...
def main(argv):
    ...
    rng = next_rng()
    def re_label(dataset, rng):
        rng, split_rng = jax.random.split(rng)
        actions, _ = rep.encoder.apply(rep.train_params['encoder'], split_rng, dataset['observations'], dataset['actions'])
        dataset['actions'] = actions
        dataset['observations'] = dataset['observations']
        return dataset
    
    new_dataset = re_label(new_dataset, rng)
    fig = plt.figure(figsize=(40, 20))
    for i in range(action_dim):
        ax1 = plt.subplot(3,6,i+1)
        # plt.figure()
        stats.probplot(dataset['actions'][:,i], dist=stats.uniform(loc=-1, scale=2), plot=pylab)
        ax1.set_title('')
        ax1.set_xlabel('')
        ax1.set_ylabel('')
        # plt.savefig('original'+FLAGS.env+'_dim_'+str(i)+'.png')

    for i in range(latent_action_dim):
        ax1 = plt.subplot(3,6,i+7)
        # plt.figure()
        stats.probplot(new_dataset['actions'][:,i], dist=stats.uniform(loc=-1, scale=2), plot=pylab)
        ax1.set_title('')
        ax1.set_xlabel('')
        ax1.set_ylabel('')
        # plt.savefig('uniform'+FLAGS.env+'_dim_'+str(i)+'.png')

    plt.savefig('original'+FLAGS.env+'.pdf')
